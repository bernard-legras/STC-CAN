...
# -*- coding: utf-8 -*-
"""
Generate history of the trajectory of Canadian vortex B2
Localize the vortex
Print the positions
...
if var=='LPV':
    trac_PV = {}
    for k in trac.keys():
        trac_PV[k] = trac[k][:169].copy()
    trac1 = trac_PV
elif var == 'O3prime':
    trac_O3 = {}
    for k in trac.keys():
        trac_O3[k] = trac[k][:169].copy()
    trac2 = trac_O3
...
pickle.dump([trac_PV,trac_O3],open('VortexB2-track.pkl','wb'))

#%% Total displacement
[trac1,trac2] = pickle.load(open('VortexB2-track.pkl','rb'))
# with PV
...
for i in range(0,169):
    ...
    cs = ax2.contour(Mof,transform=projplate,extent=(dats[i+ioff].attr['lons'][0]-cm_lon,
                dats[i+ioff].attr['lons'][-1]-cm_lon,dats[i+ioff].attr['lats'][0],dats[i+ioff].attr['lats'][-1]),origin='lower')
    plt.clabel(cs)
    ax2.plot(trac2['lons'][i]-cm_lon,trac2['lats'][i],'black',marker='x',ms=21,mew=2)
    ax1.text(-0.05,1.05,'a)',transform=ax1.transAxes,fontsize=22)
    ax2.text(-0.05,1.05,'b)',transform=ax2.transAxes,fontsize=22)
    if movieOn:
        plt.savefig('tmpimB2.png',bbox_inches='tight')
        images.append(imageio.imread('tmpimB2.png'))
    plt.show()
#%%
if movieOn:
    imageio.mimsave('movie_VortexB2.gif', images, fps=2)
    ...
    imageio.mimsave('movie_VortexB2.mp4', images, fps=2)
#%% Total displacement
...
fig.suptitle('Canadian event   vortex B2 tracking')
fig.autofmt_xdate()
if figsav:
    plt.savefig(join('VortexMotion_VortexB2_ERA5PV.png'),**figargs)
plt.show()
