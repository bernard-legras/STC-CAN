...
for i in range(0,224):
    ...
    Mo = (cst.g * dats[i].var['Z'] + cst.Cp * dats[i].var['T']) * 1.e-6
    kz1 = trac1['kz'][i]
    kz2 = trac2['kz'][i]
    ...
    ax1 = dats[i].show('PV',kz1,figsize=None,axf=ax1,show=False,scale=1.e6,xylim=True,
                 txt='PV '+str(i)+'  '+str(kz1)+trac1['dates'][i].strftime('  %Y %b %d %HUTC  ')+str(pts[kz1])+'K'+
    ...
    ax2 = dats[i].show('O3',kz2,figsize=None,axf=ax2,show=False,scale=1.e6,xylim=True,
                 txt='O3 '+str(i)+'  '+str(kz2)+trac2['dates'][i].strftime('  %Y %b %d %HUTC  ')+str(pts[kz2])+'K'+
